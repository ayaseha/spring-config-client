...
public class BoardController {
	...
	//게시글페이지로 이동하면서 db에서 모든데이터 갖고 표현
	@GetMapping("/board")
	...
	//@GetMapping("/board") 동일한url이나 method가 다르면 허용
	@PostMapping("/board")
	public String write(BoardSaveDTO dto) {//파라미터 매핑
		//JSP에서 String title=request.getParameter("title"); DTO로 처리 
		service.save(dto);
	...
	public String delete(@PathVariable("no") long bno) {//삭제할 pk값이 no -> bno
		service.delete(bno);
		return "redirect:/board";
		//redirect: ->웹페이지의 url 변경요청 /board
	}

...
//	private String kakaoKey;
	...
	@Override
	public void busStopLocationXyInfo(Model model) throws Exception {
//		System.out.println(kakaoKey);
//		//http://openapi.seoul.go.kr:8088/(인증키)/xml/busStopLocationXyInfo/1/5/
...
//		
	...
	public void getElevatorInfo(Model model) throws Exception {
		...
		model.addAttribute("list", result.getSubwayTourInfo().getRow().stream()
									.sorted(Comparator.comparing(Item::getLINE_NAME).thenComparing(Comparator.comparing(Item::getSTATION)))
									.filter(item->item.getLINE().equals("4"))
									.collect(Collectors.toList()));
	}

	//응답데이터를 스트림을 통해서 한줄씩읽어서 문자열로 리턴해주는 메서드
